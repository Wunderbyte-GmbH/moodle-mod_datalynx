...
class behat_mod_datalynx extends behat_base {
    ...
    /**
     * Clicks the delete button for a specific visible fieldgroup row.
     *
     * @When /^I delete the fieldgroup row "(?P<row_number>\d+)"$/
     *
     * @param int $row
     * @throws coding_exception
     * @throws Exception
     */
    public function i_delete_the_fieldgroup_row($row) {
        if (!$this->running_javascript()) {
            throw new coding_exception('This step requires JavaScript.');
        }
        ...
        $result = $this->getSession()->evaluateScript(
            ...
            "  });" .
            "  if (!line) { return 'visible-row-not-found:' + row; }" .
            "  var button = line.querySelector('[data-removeline]');" .
            "  if (!button) { return 'delete-button-not-found:' + row; }" .
            "  button.click();" .
            "  return window.getComputedStyle(line).display === 'none' ? 'hidden' : 'visible';" .
            "})($row);"
        );

        if (!in_array($result, ['hidden', 'visible'], true)) {
            throw new Exception("Unable to delete fieldgroup row: {$result}");
        }
    }

    /**
     * Verifies a checkbox option is checked inside a visible fieldgroup row.
     *
     * @Then /^the "(?P<option_string>(?:[^"]|\\")*)" option should be checked for the "(?P<fieldname_string>(?:[^"]|\\")*)" fieldgroup field in row "(?P<row_number>\d+)"$/
     *
     * @param string $option
     * @param string $fieldname
     * @param int $row
     ...
     * @throws Exception
     */
    public function the_option_should_be_checked_for_the_fieldgroup_field_in_row($option, $fieldname, $row) {
        if (!$this->running_javascript()) {
            throw new coding_exception('This step requires JavaScript.');
        }
        ...
        $result = $this->getSession()->evaluateScript(
            ...
            "    return element.textContent.trim() === option;" .
            "  });" .
            "  if (!label) { return 'option-not-found:' + option; }" .
            "  var input = label.querySelector('input[type=\"checkbox\"]');" .
            "  return input && input.checked ? 'checked' : 'unchecked';" .
            "})($fieldnamejson, $optionjson, $row);"
        );

        if ($result !== 'checked') {
            throw new Exception("Expected checked fieldgroup checkbox option, got: {$result}");
        }
    }

    /**
     * Verifies a fieldgroup row is hidden.
     *
     * @Then /^fieldgroup row "(?P<row_number>\d+)" should be hidden$/
     *
     * @param int $row
     * @throws coding_exception
     * @throws Exception
     */
    public function fieldgroup_row_should_be_hidden($row) {
        if (!$this->running_javascript()) {
            throw new coding_exception('This step requires JavaScript.');
        }
        ...
        $result = $this->getSession()->evaluateScript(
            ...
            "  if (!fieldgroup) { return 'fieldgroup-not-found'; }" .
            "  var line = Array.from(fieldgroup.querySelectorAll('.lines')).find(function(element) {" .
            "    return parseInt(element.dataset.line, 10) === row;" .
            "  });" .
            "  if (!line) { return 'row-not-found:' + row; }" .
            "  return window.getComputedStyle(line).display === 'none' ? 'hidden' : 'visible';" .
            "})($row);"
        );

        if ($result !== 'hidden') {
            throw new Exception("Expected hidden fieldgroup row, got: {$result}");
        }
    }
