...
class behat_mod_datalynx extends behat_files {
     ...
     *
     * @Given /^I create a "(?P<fieldname_string>(?:[^"]|\\")*)" field in "(?P<activityname_string>(?:[^"]|\\")*)" instance and fill the form with:$/
     *
     ...
     * @return Given[]
     */
    public function i_create_a_field_in_instance_and_fill_the_form_with($fieldtype, $activityname, TableNode $fielddata) {
        $steps = array(
            new Given('I follow "' . $this->escape($activityname) . '"'),
            new Given('I follow "Manage"'),
            new Given('I follow "Fields"'),
     ...
     *
     * @Given /^I update "(?P<fieldname_string>(?:[^"]|\\")*)" field in "(?P<activityname_string>(?:[^"]|\\")*)" instance and fill the form with:$/
     *
     ...
     * @return Given[]
     */
    public function i_update_field_in_instance_and_fill_the_form_with($fieldname, $activityname, TableNode $fielddata) {
        $steps = array(
            new Given('I follow "' . $this->escape($activityname) . '"'),
            new Given('I follow "Manage"'),
            new Given('I follow "Fields"'),
     ...
     *
     * @Given /^I delete "(?P<fieldname_string>(?:[^"]|\\")*)" field in "(?P<activityname_string>(?:[^"]|\\")*)" instance$/
     *
     * @param string $fieldname,
     * @param string $activityname
     * @return Given[]
    ...
    public function i_click_button_of_item($button, $fieldname) {
        ...
        $element = $session->getPage()->find('xpath', '//a[text()="' . $this->escape($fieldname) . '"]/ancestor::tr//a/img[@title="' . $this->escape($button) . '"]/ancestor::a');
        $element->click();
    }
     ...
     *
     * @Given /^I create a "(?P<viewtype_string>(?:[^"]|\\")*)" view in "(?P<activityname_string>(?:[^"]|\\")*)" instance and fill the form with:$/
     *
     ...
     *
     * @Given /^I view "(?P<viewname_string>(?:[^"]|\\")*)" view in "(?P<activityname_string>(?:[^"]|\\")*)" instance$/
     *
     ...
     * Updates a field in the list
     *
     * @Given /^I update "(?P<viewname_string>(?:[^"]|\\")*)" view in "(?P<activityname_string>(?:[^"]|\\")*)" instance and fill the form with:$/
     *
     * @param string $viewname,
     * @param string $activityname,
     * @param TableNode $fielddata
     ...
     *
     * @Given /^I delete "(?P<viewname_string>(?:[^"]|\\")*)" view in "(?P<activityname_string>(?:[^"]|\\")*)" instance$/
     *
     * @param string $viewname,
     * @param string $activityname
     * @return Given[]
     ...
     * Deletes a view in the list
     *
     * @Given /^I set "(?P<viewname_string>(?:[^"]|\\")*)" view in "(?P<activityname_string>(?:[^"]|\\")*)" instance as detailed view$/
     *
     * @param string $viewname,
     * @param string $activityname
     * @return Given[]
    ...
    public function i_set_view_as_detailed_view_in_instance($viewname, $activityname) {
        $steps = array(
            ...
            new Given('I follow "Manage"'),
            new Given('I follow "Views"'),
            new Given('I click "Set as detailed view" button of "' . $this->escape($viewname) . '" item'),
        );
    ...
    public function i_select_item($itemname) {
        $session = $this->getSession(); // get the mink session
        $element = $session->getPage()->find('xpath', '//a[text()="' . $this->escape($itemname) . '"]/ancestor::tr//input[@type="checkbox"]');
    ...
    public function i_click_action_button($actionname) {
        $session = $this->getSession(); // get the mink session
        $element = $session->getPage()->find('xpath', '//button[@name="' . $this->escape($actionname) . '"]');
     ...
     *
     * @Given /^I set "(?P<filtername_string>(?:[^"]|\\")*)" filter to "(?P<viewname_string>(?:[^"]|\\")*)" view in "(?P<activityname_string>(?:[^"]|\\")*)" instance$/
     *
     ...
     *
     * @Given /^I select "(?P<filtername_string>(?:[^"]|\\")*)" filter for "(?P<viewname_string>(?:[^"]|\\")*)" view$/
     *
     * @param string $filtername
     * @param string $viewname
     */
    public function i_select_filter_for_view($filtername, $viewname) {
        $session = $this->getSession(); // get the mink session
        $element = $session->getPage()->find('xpath', '//a[text()="' . $this->escape($viewname) . '"]/ancestor::tr//select');
     ...
     * Creates a new view in a datalynx instance
     *
     * @Given /^I create a filter in "(?P<activityname_string>(?:[^"]|\\")*)" instance and fill the form with:$/
     *
     * @param string $activityname,
     * @param TableNode $fielddata
     * @return Given[]
     ...
     *
     * @Given /^I create an entry in "(?P<activityname_string>(?:[^"]|\\")*)" instance and fill the form with:$/
     *
     * @param string $activityname,
     * @param TableNode $fielddata
     * @return Given[]
    ...
    public function i_edit_entry($entrynumber) {
        ...
        $session = $this->getSession(); // get the mink session
        $element = $session->getPage()->find('xpath', '//div[@class="entry"][' . $number . ']//img[@alt="Edit"]//ancestor::a');
        $element->click();
    }
    ...
    public function i_delete_entry($entrynumber) {
        ...
        $session = $this->getSession(); // get the mink session
        $element = $session->getPage()->find('xpath', '//div[@class="entry"][' . $number . ']//img[@alt="Delete"]//ancestor::a');
        $element->click();
    }
     ...
     *
     * @Given /^I create an entry in "(?P<viewtype_string>(?:[^"]|\\")*)" view in "(?P<activityname_string>(?:[^"]|\\")*)" instance and fill the form with:$/
     *
     * @param string $viewtype
     * @param string $activityname
     * @param TableNode $fielddata
     * @return Given[]
     */
    public function i_create_an_entry_in_view_in_instance($viewtype, $activityname, TableNode $fielddata) {
        $steps = array(
            new Given('I follow "' . $this->escape($activityname) . '"'),
    ...
    /**
     * Sets up fields for the given datalynx instance. Optional, but must be used after instance declaration.
     *
    ...
    /**
     * Sets up filters for the given datalynx instance. Optional, but must be used after field setup.
     *
    ...
    /**
     * Sets up filters for the given datalynx instance. Optional, but must be called after field and filter setup.
     *
    ...
    /**
     * Sets up entries for the given datalynx instance. Optional, but must be called after field setup.
     *
    ...
    public function i_select_entry($entrynumbers) {
        ...
        foreach($entrynumbers as $entrynumber) {
            ...
            $element = $session->getPage()->find(
                'xpath', '//div[@class="entriesview"]/table/tbody/tr[' . $number . ']//input[@type="checkbox"]');
    ...
    public function i_fill_entry_form_with(TableNode $table) {
        ...
        foreach ($entries as $number => $entry) {
            $entryelement = $session->getPage()->find(
                'xpath', '//div[@class="entriesview"]/table/tbody/tr[' . $number . '] | //div[@class="entry"][' . $number . ']');
    ...
    private function map_view_names_for_redirect($views, $names) {
        global $DB;
        foreach ($views as $view) {
            $DB->set_field('datalynx_views', 'param10', $names[$view['redirect']], array('id' => $view['id']));
        }
    }
    ...
    private function create_field($record = null, array $options = null) {
        ...
        $defaults = array (
            ...
            'param3' => null,
            'param4' => ($record->type == "teammemberselect") ? "4" : null,
            'param5' => null,
            'param6' => null,
            'param7' => null,
            'param8' => null,
        ...
        if (isset($record->param1) && ($record->type == "radiobutton" || $record->type == "select" || $record->type == "checkbox")) {
...
",
    ...
    private function create_view($record = null, array $options = null) {
        ...
        $defaults = array (
            ...
            'param1' => null,
            'param2' => null,
            'param3' => ($record->type == "tabular") ? 1 : null,
            'param4' => null,
            'param5' => null,
            'param6' => null,
            'param7' => null,
