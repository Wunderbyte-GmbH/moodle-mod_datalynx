...
navigation_node::override_active_url(new moodle_url('/mod/datalynx/renderer/index.php', array('id' => $datalynx->cm->id)));
...
echo html_writer::link(new moodle_url('/mod/datalynx/renderer/renderer_edit.php', array('d' => $datalynx->id(), 'sesskey' => sesskey(), 'id' => 0)), get_string('rendereradd', 'datalynx'));
...
$table->define_baseurl(new moodle_url('/mod/datalynx/renderer/index.php', array('d' => $datalynx->id())));
...
foreach ($renderers as $rendererid => $renderer) {
    ...
    $fieldname = html_writer::link(new moodle_url($editbaseurl, $linkparams + array('id' => $rendererid)), $renderer->name);
    $fielddescription = shorten_text($renderer->description, 30);
    $fieldedit = html_writer::link(new moodle_url($editbaseurl, $linkparams + array('id' => $rendererid)), $OUTPUT->pix_icon('t/edit', get_string('edit')));
    $fieldduplicate = html_writer::link(new moodle_url($editbaseurl, $linkparams + array('action' => 'duplicate', 'id' => $rendererid)), $OUTPUT->pix_icon('t/copy', get_string('duplicate')));
    $fielddelete = html_writer::link(new moodle_url($editbaseurl, $linkparams + array('action' => 'delete', 'id' => $rendererid)), $OUTPUT->pix_icon('t/delete', get_string('delete')));
