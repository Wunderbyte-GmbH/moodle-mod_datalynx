...
class datalynx_preset_manager {
     ...
     */
    public function __construct($dlx) {
        $this->dlx = $dlx;
    }
    ...
    public function get_user_presets($presetarea) {
        ...
        $presets = [];
        $coursecontext = context_course::instance($this->dlx->course->id);
        ...
        $canviewall = has_capability('mod/datalynx:presetsviewall', $this->dlx->context);
        if (!empty($files)) {
    ...
    public function print_presets_list($localpresets, $sharedpresets) {
        ...
        if ($localpresets || $sharedpresets) {
            $linkparams = ['d' => $this->dlx->id(), 'sesskey' => sesskey()];
            ...
            $selectallnone = html_writer::checkbox(
                ...
                false,
                null,
                ['onclick' => 'select_allnone(\'preset\'&#44;this.checked)']
            );

            $multidownload = html_writer::tag(
                'button',
            ...
            $table->head = [$strname, $strdescription, $strscreenshot, $strapply,
                    $multidownload, $multishare, $multidelete, $selectallnone];
            $table->align = ['left', 'left', 'center', 'center', 'center', 'center', 'center', 'center'];
            ...
            if ($localpresets) {
                ...
                $lpheadingcell->text = html_writer::tag(
                    'h4',
                ...
                foreach ($localpresets as $preset) {
                    ...
                    $presetapply = html_writer::link(
                        new moodle_url($targetpage, $linkparams + ['apply' => $preset->id]),
                        $OUTPUT->pix_icon('t/switch_whole', $strapply)
                    );
                    $presetdownload = html_writer::link(
                        moodle_url::make_file_url(
                            "/pluginfile.php",
                    ...
                    $presetshare = '';
                    if (has_capability('mod/datalynx:presetsviewall', $this->dlx->context)) {
                        $presetshare = html_writer::link(
                            new moodle_url(
                                $targetpage,
                                $linkparams + ['share' => $preset->id]
                            ),
                            $OUTPUT->pix_icon('i/group', $strshare)
                        );
                    }
                    $presetdelete = html_writer::link(
                        new moodle_url($targetpage, $linkparams + ['delete' => $preset->id]),
                        $OUTPUT->pix_icon('t/delete', $strdelete)
                    );
                    $presetselector = html_writer::checkbox("presetselector", $preset->id, false);

                    $table->data[] = [$presetname, $presetdescription, $presetscreenshot,
            ...
            if ($sharedpresets) {
                ...
                foreach ($sharedpresets as $preset) {
                    ...
                    $presetshare = '';
                    $presetdelete = '';
                    if (has_capability('mod/datalynx:managepresets', $this->dlx->context)) {
                        $presetdelete = html_writer::link(
                            new moodle_url(
                                $targetpage,
                                $linkparams + ['delete' => $preset->id]
                            ),
                            $OUTPUT->pix_icon('t/delete', $strdelete)
                        );
                    }
    ...
    public function print_preset_form() {
        ...
        $mform = new mod_datalynx_preset_form(
            new moodle_url(
                ...
                ['d' => $this->dlx->id(), 'sesskey' => sesskey(), 'add' => 1]
            )
        );
        $mform->set_data(null);
        $mform->display();
    ...
    public function process_presets($params) {
        ...
        if ($data = $mform->get_data()) {
            ...
            if ($params->apply && confirm_sesskey()) {
                ...
                rebuild_course_cache($this->dlx->course->id);
    ...
    public function create_preset_from_backup($userdata) {
        ...
        $SESSION->{"datalynx_{$this->dlx->cm->id}_preset"} = "$users $anon";
        ...
        unset($SESSION->{"datalynx_{$this->dlx->cm->id}_preset"});
        ...
        if (!empty($files)) {
            ...
            foreach ($files as $file) {
                ...
                $preset->filename = clean_filename(
                    str_replace(' ', '_', $this->dlx->data->name) . '-datalynx-preset-' .
                        gmdate("Ymd_Hi") . '-' .
    ...
    public function apply_preset($userpreset, $torestorer = true) {
        ...
        $folder = 'tmp-' . $this->dlx->context->id . '-' . time();
        ...
        if ($datalynxtask) {
            $datalynxtask->set_activityid($this->dlx->id());
            $datalynxtask->set_moduleid($this->dlx->cm->id);
            $datalynxtask->set_contextid($this->dlx->context->id);
            if ($torestorer) {
            ...
            redirect(new moodle_url('/mod/datalynx/view.php', ['d' => $this->dlx->id()]));
        } else {
            $rc->destroy();
        }
    }
