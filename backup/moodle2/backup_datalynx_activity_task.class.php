...
class backup_datalynx_activity_task extends backup_activity_task {
    ...
    public static function encode_content_links($content) {
        ...
        $replacement = ['$@DFVIEWVIEWFILTER*$2*$4*$6@$', '$@DFEMBEDVIEWFILTER*$2*$4*$6@$'];
        ...
        $replacement = ['$@DFVIEWVIEW*$2*$4@$', '$@DFEMBEDVIEW*$2*$4@$'];
        $content = preg_replace($search, $replacement, $content);
        ...
        $replacement = ['$@DFVIEWENTRY*$2*$4@$', '$@DFEMBEDENTRY*$2*$4@$'];
        $content = preg_replace($search, $replacement, $content);

        // View/embed: id.
        $search = ["/(" . $base . "\/mod\/datalynx\/view.php\?id\=)([0-9]+)/",
                "/(" . $base . "\/mod\/datalynx\/embed.php\?id\=)([0-9]+)/"];
        $replacement = ['$@DFVIEWBYID*$2@$', '$@DFEMBEDBYID*$2@$'];
        $content = preg_replace($search, $replacement, $content);

        // View/embed: d.
        $search = ["/(" . $base . "\/mod\/datalynx\/view.php\?d\=)([0-9]+)/",
                "/(" . $base . "\/mod\/datalynx\/embed.php\?d\=)([0-9]+)/"];
        $replacement = ['$@DFVIEWBYD*$2@$', '$@DFEMBEDBYD*$2@$'];
        $content = preg_replace($search, $replacement, $content);
