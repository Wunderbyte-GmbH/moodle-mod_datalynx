...
if (isloggedin() && $istemplatemanager) {
    ...
    $row[] = new tabobject(
        'browse',
        new moodle_url('/mod/datalynx/view.php', ['d' => $this->id()]),
        get_string('browse', 'datalynx')
    );
    $row[] = new tabobject(
        'manage',
        new moodle_url('/mod/datalynx/view/index.php', ['d' => $this->id()]),
        get_string('manage', 'datalynx')
    );
    // Add view edit tab.
    if ($currenttab == 'browse' && !empty($this->currentview)) {
        $params = ['d' => $this->id(), 'sesskey' => sesskey(),
                'vedit' => $this->currentview->id()];
        $editviewurl = new moodle_url('/mod/datalynx/view/view_edit.php', $params);
        $row[] = new tabobject(
            'editview',
            $editviewurl,
            $OUTPUT->pix_icon('t/edit', get_string('vieweditthis', 'datalynx')),
            get_string('vieweditthis', 'datalynx')
        );
    }
    ...
    if ($currenttab != 'browse') {
        ...
        if ($istemplatemanager) {
            ...
            $row[] = new tabobject(
                'filters',
                new moodle_url('/mod/datalynx/filter/index.php', ['d' => $this->id()]),
                get_string('filters', 'datalynx')
            );
            $row[] = new tabobject(
                'customfilters',
                new moodle_url('/mod/datalynx/customfilter/index.php', ['d' => $this->id()]),
                get_string('customfilters', 'datalynx')
            );
            $row[] = new tabobject(
                'rules',
                new moodle_url('/mod/datalynx/rule/index.php', ['d' => $this->id()]),
                get_string('rules', 'datalynx')
            );
            $row[] = new tabobject(
                'tools',
                new moodle_url('/mod/datalynx/tool/index.php', ['d' => $this->id()]),
                get_string('tools', 'datalynx')
            );
            $row[] = new tabobject(
                'js',
                new moodle_url('/mod/datalynx/js.php', ['d' => $this->id(), 'jsedit' => 1]),
                get_string('jsinclude', 'datalynx')
            );
            $row[] = new tabobject(
                'css',
                new moodle_url('/mod/datalynx/css.php', ['d' => $this->id(), 'cssedit' => 1]),
                get_string('cssinclude', 'datalynx')
            );
            $row[] = new tabobject(
                'presets',
                new moodle_url('/mod/datalynx/preset/index.php', ['d' => $this->id()]),
                get_string('presets', 'datalynx')
            );
            $row[] = new tabobject(
                'import',
                new moodle_url('/mod/datalynx/import.php', ['d' => $this->id()]),
                get_string('import', 'datalynx')
            );
            $row[] = new tabobject(
                'statistics',
                new moodle_url('/mod/datalynx/statistics/index.php', ['d' => $this->id()]),
    ...
    if (
        $currenttab == 'fields' || $currenttab == 'fields2' || $currenttab == 'behaviors' ||
            $currenttab == 'renderers' || $currenttab == 'fieldgroups' || $currenttab == 'formats'
    ) {
        $inactive[] = 'fields';
        $activated[] = 'fields';
        ...
        if ($istemplatemanager) {
            ...
            $row[] = new tabobject(
                ...
                get_string('formats', 'datalynx')
            );
            $row[] = new tabobject(
                'fieldgroups',
                new moodle_url('/mod/datalynx/field/fieldgroup/index.php', ['d' => $this->id()]),
