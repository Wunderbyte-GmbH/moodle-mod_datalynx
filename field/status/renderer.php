 ...
 */
class datalynxfield_status_renderer extends datalynxfield_renderer {
    /**
    ...
    public function display_edit(&$mform, $entry, array $options = []) {
        ...
        $fieldid = $field->id();
        $entryid = $entry->id;
        $status = isset($entry->status) ? $entry->status : datalynxfield_status::STATUS_NOT_SET;
    ...
    private function menu_status($shownotset = false) {
        if ($shownotset) {
            return [
                    datalynxfield_status::STATUS_NOT_SET => get_string('statusnotcreated', 'datalynx'),
                    datalynxfield_status::STATUS_DRAFT => get_string('statusdraft', 'datalynx'),
                    datalynxfield_status::STATUS_FINAL_SUBMISSION => get_string(
                        'statusfinalsubmission',
                        'datalynx'
                    ),
            ];
        } else {
            return [datalynxfield_status::STATUS_NOT_SET => get_string('choosedots'),
                    datalynxfield_status::STATUS_DRAFT => get_string('statusdraft', 'datalynx'),
                    datalynxfield_status::STATUS_FINAL_SUBMISSION => get_string(
                        'statusfinalsubmission',
                        'datalynx'
    ...
    protected function display_browse($entry, $params = []) {
        ...
        if (isset($entry) && isset($entry->status)) {
            ...
            return $menu[datalynxfield_status::STATUS_NOT_SET];
        }
    }
