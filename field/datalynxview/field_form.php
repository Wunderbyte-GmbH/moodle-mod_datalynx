...
class datalynxfield_datalynxview_form extends datalynxfield_form {
    ...
    //-------------------------------------------------------------------------------
        ...
        // Filters menu
        $options = array(0 => get_string('choosedots'));
        $mform->addElement('select', 'param3', get_string('filter', 'datalynxfield_datalynxview'), $options);
        ...
        $grp = array();
        $grp[] = &$mform->createElement('advcheckbox', 'entryauthor', null, get_string('entryauthor', 'datalynxfield_datalynxview'), null, array(0,1));
        $grp[] = &$mform->createElement('advcheckbox', 'entrygroup', null, get_string('entrygroup', 'datalynxfield_datalynxview'), null, array(0,1));
        $mform->addGroup($grp, 'filterbyarr', get_string('filterby', 'datalynxfield_datalynxview'), '<br />', false);
        $mform->addHelpButton('filterbyarr', 'filterby', 'datalynxfield_datalynxview');

        // Custom sort options
        $mform->addElement('textarea', 'param4', get_string('customsort', 'datalynxfield_datalynxview'), array('rows'=>'5', 'cols'=>'60'));
        ...
        $mform->addElement('textarea', 'param5', get_string('customsearch', 'datalynxfield_datalynxview'), array('rows'=>'5', 'cols'=>'60'));
    ...
    function definition_after_data() {
        ...
        if ($datalynxid) {           
            ...
            if ($viewid) {           
                if ($filters = $DB->get_records_menu('datalynx_filters', array('dataid' => $datalynxid), 'name', 'id,name')) {
