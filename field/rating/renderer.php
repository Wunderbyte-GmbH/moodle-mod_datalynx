 ...
 * @package    mod_datalynx
 */
class datalynxfield_rating_renderer extends datalynxfield_renderer {
    /**
    ...
    public function replacements(array $tags = null, $entry = null, array $options = null) {
        ...
        if ($entry->id > 0) {
            ...
            $aggravg = round($entry->rating->aggregate[datalynxfield_rating::AGGREGATE_AVG] ?: 0, 2);
            $aggrmax = round($entry->rating->aggregate[datalynxfield_rating::AGGREGATE_MAX] ?: 0, 2);
            $aggrmin = round($entry->rating->aggregate[datalynxfield_rating::AGGREGATE_MIN] ?: 0, 2);
            $aggrsum = round($entry->rating->aggregate[datalynxfield_rating::AGGREGATE_SUM] ?: 0, 2);
    ...
    public function get_aggregations($patterns) {
        $aggr = [datalynxfield_rating::AGGREGATE_AVG => '##ratings:avg##',
                datalynxfield_rating::AGGREGATE_MAX => '##ratings:max##',
                datalynxfield_rating::AGGREGATE_MIN => '##ratings:min##',
                datalynxfield_rating::AGGREGATE_SUM => '##ratings:sum##'];
        ...
        if (isset($entry->rating)) {
            ...
            if (
                ...
                if (!$rating->settings->scale->isnumeric) {
                    $ratinghtml .= $OUTPUT->help_icon_scale(
                        $rating->settings->scale->courseid,
