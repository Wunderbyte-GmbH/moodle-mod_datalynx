...
class renderer extends datalynxfield_renderer {
    ...
    public function replacements(array $tags = null, $entry = null, array $options = null) {
        ...
        if ($entry->id > 0) {
            ...
            $aggravg = round($entry->rating->aggregate[field::AGGREGATE_AVG] ?: 0, 2);
            $aggrmax = round($entry->rating->aggregate[field::AGGREGATE_MAX] ?: 0, 2);
            $aggrmin = round($entry->rating->aggregate[field::AGGREGATE_MIN] ?: 0, 2);
            $aggrsum = round($entry->rating->aggregate[field::AGGREGATE_SUM] ?: 0, 2);
    ...
    public function get_aggregations($patterns) {
        $aggr = [field::AGGREGATE_AVG => '##ratings:avg##',
                field::AGGREGATE_MAX => '##ratings:max##',
                field::AGGREGATE_MIN => '##ratings:min##',
                field::AGGREGATE_SUM => '##ratings:sum##'];
