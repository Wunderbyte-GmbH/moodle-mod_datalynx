...
class form extends datalynxfield_form {
    ...
    public function field_definition() {
        ...
        $mform->addElement('text', 'param3', get_string('minteamsize', 'datalynx'), ['size' => 3]);
        $mform->addHelpButton('param3', 'minteamsize', 'datalynx');
        $mform->setType('param3', PARAM_INT);
        $mform->addRule(['param3', 'param1'], get_string('minteamsizeerrorvalue', 'datalynx'), 'compare', 'lte');
        $mform->setDefault('param3', 0);

        // Admissible roles.
        $group = [];
        $permissions = $this->dlx->get_datalynx_permission_names(true);
        ...
        $mform->setDefault('param4', field::TEAMMEMBERSELECT_FORMAT_UL);
        ...
        $fieldmenu = $this->dlx->get_fields(array_keys($this->dlx->get_internal_fields()), true);
