...
class datalynx {
    ...
    public function set_page($page = 'view', $params = null, $skiplogincheck = false) {
        ...
        if ($manager) {
            ...
            if (empty($viewrecords)) {
                ...
                $this->notifications['bad']['defaultview'] = get_string('viewnodefault', 'datalynx', 
                        $linktoviews);
            }
        }
    ...
    public function print_rsslink() {
        ...
        if (!empty($CFG->enablerssfeeds) && !empty($CFG->datalynx_enablerssfeeds) &&
            ...
            rss_print_link($this->course->id, $USER->id, 'datalynx', $this->id(), 
                    get_string('rsstype'));
            echo '</div>';
    ...
    public function print_intro() {
        ...
        if ($this->data->intro) {
            ...
            echo $OUTPUT->box(format_module_intro('datalynx', $this->data, $this->cm->id), 
                    'generalbox', 'intro');
        }
    }
    ...
    public function get_fields($exclude = null, $menu = false, $forceget = false, $sort = '') {
        ...
        if (!$this->fields or $forceget) {
            $this->fields = array();
            // collate user fields
            if ($fields = $DB->get_records('datalynx_fields', array('dataid' => $this->id()
            ), $sort)) {
    ...
    private function find_filters_using_fields($fields) {
        ...
        foreach ($filters as $filter) {
            ...
            if (array_intersect($fieldids, $customsearch) || array_intersect($fieldids, 
                    $custormsort)) {
                $usedfilters[] = $filter;
            }
        }
        ...
        if (empty($fields) and $action != 'add') {
            ...
            if (!$confirmed) {
                            ...
                            if ($contents) {
                                foreach ($contents as $contentid => $content) {
                                    if (!$this->is_html($content->content)) {
                                        ...
                                        $htmlcontent->content = format_text($content->content, 
                                                FORMAT_PLAIN);
                ...
                if ($strnotify) {
                    ...
                    $this->notifications['good'][] = get_string($strnotify, 'datalynx', 
                            $fieldsprocessed);
                }
