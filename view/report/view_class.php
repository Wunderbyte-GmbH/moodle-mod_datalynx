    ...
    public function display_entries(?array $options = null): string {
        ...
        if ($this->view->param2 === 'month') {
            ...
            foreach ($monthlydatabymonth as $month => $usersdata) {
                ...
                foreach ($usersdata as $userid => $usermonthdata) {
                    // Total entries for this user in this month.
                    $usertotalentries = $usermonthdata['totalentries'];
                    ...
                    foreach ($desiredfieldoptions as $label) {
                        $count = $usermonthdata['matchingcontents'][$label] ?? 0;
                        if ($count !== 0) {
            ...
            foreach ($monthlydatabymonth as $month => $usersdata) {
                ...
                foreach ($usersdata as $userid => $usermonthdata) {
                    $output .= html_writer::start_tag('tr');
                    $output .= html_writer::tag('td', $usermonthdata['userinfo']);
    ...
    public function create_report_sql(): array {
        ...
        foreach ($userarray as $userid => $entryids) {
            ...
            foreach ($entryids as $entryid) {
                ...
                $entryrecord = $DB->get_record('datalynx_entries', ['id' => $entryid, 'dataid' => $dataid], 'id, timecreated');
                if ($entryrecord) {
                    $month = date('Y-m', $entryrecord->timecreated);
                    ...
                    foreach ($desiredfieldoptions as $value => $label) {
                        ...
                        $matchingcontentscount = $DB->count_records_sql($sql, $params);
                        ...
                        $report[$userid]['monthly'][$month]['matchingcontents'][$label] += $matchingcontentscount;
                    }
                }
            }
