...
abstract class datalynxview_base {
 ...
 // The datalynx object that this view belongs to
    ...
    public function __construct($df = 0, $view = 0, $filteroptions = true) {
        ...
        $this->_baseurl = new moodle_url("/mod/datalynx/{$this->_df->pagefile()}.php", 
                $baseurlparams);
    ...
    public function get_default_view_settings() {
        return (object) array('description' => '', 'visible' => 7, 'perpage' => 0, 'groupby' => '', 
            'filter' => 0
        );
    }
    ...
    protected function set_groupby_per_page() {
        ...
        $sortfields = array($fieldid => $sortdir
        ) + $sortfields;
        ...
        foreach ($elements as $element) {
            if (!empty($element)) {
                ...
                if ($type === 'html') {
                    ...
                    call_user_func_array($func, array_merge(array($mform
                    ), $params));
                }
            }
        }
