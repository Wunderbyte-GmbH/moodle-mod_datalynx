...
switch ($urlparams->action) {
    case "edit":
        ...
        $datalynx->print_header(array('tab' => 'filterforms', 'nonotifications' => true, 'urlparams' => $urlparams));

        if ($urlparams->id) {
            $data = mod_datalynx_advanced_filter_form::get_filterform($urlparams->id);
            $mform->set_data($data);
            echo html_writer::tag('h2', get_string('editingfilterform', 'datalynx', $data->name), array('class' => 'mdl-align'));
        } else {
            echo html_writer::tag('h2', get_string('newfilterform', 'datalynx'), array('class' => 'mdl-align'));
        }
    ...
    case "duplicate":
        if ($urlparams->confirmed) {
            ...
            $datalynx->print_header(array('tab' => 'filterforms', 'nonotifications' => true, 'urlparams' => $urlparams));
            echo html_writer::tag('h2', get_string('duplicatingfilterform', 'datalynx', $data->name), array('class' => 'mdl-align'));
            echo $OUTPUT->confirm(get_string('confirmfilterformduplicate', 'datalynx'), new moodle_url('filterform_edit.php', (array) $urlparams), $returnurl);
            $datalynx->print_footer();
        }
        break;
    ...
    case "delete":
        if ($urlparams->confirmed) {
            ...
            $urlparams->confirmed = true;
            $datalynx->print_header(array('tab' => 'filterforms', 'nonotifications' => true, 'urlparams' => $urlparams));
            echo html_writer::tag('h2', get_string('deletingfilterform', 'datalynx', $data->name), array('class' => 'mdl-align'));
            echo $OUTPUT->confirm(get_string('confirmfilterformdelete', 'datalynx'), new moodle_url('filterform_edit.php', (array) $urlparams), $returnurl);
            $datalynx->print_footer();
        }
        break;
