...
class datalynx_field_filterform_form extends moodleform {
    ...
    protected function definition() {
        ...
        $mform->addRule('name', "Filter form name may not contain the pipe symbol \" | \"!", 'regex', '/^[^\|]+$/', 'client');

        $mform->addElement('text', 'description', get_string('description'), array('size' => '64'));
        $mform->setType('description', PARAM_TEXT);

        $mform->addElement('checkboxgroup', 'fields', get_string('fields', 'datalynx'),
                            array_map(function ($field) {return $field->field->name;}, $this->datalynx->get_fields()), '<br>');
