...
class datalynx_entries {
    ...
    public function get_entries($options = null) {
        ...
        // Filter sql.
        [$filtertables, $wheresearch, $sortorder, $whatcontent, $filterparams, $datalynxcontent] = $filter->get_sql($fields);
        ...
        if (isset($optionseids)) { // Datalynxview_field's entry-ids as an additional filter.
            ...
            [$ineids, $eidparams] = $DB->get_in_or_equal($eids, SQL_PARAMS_NAMED, 'eid');
                                    ...
                                    foreach ($contents[$eid]['fields'] as $fieldid => $content) {
                                        ...
                                        if (strpos($fieldgroup, "fieldgroup") === 0) {
                                            ...
                                            foreach ($content as $key => $value) {
                                                ...
                                                if (
                                                        !isset($entry->{"c{$fieldid}_id_fieldgroup"})
    ...
    public function update_entry($entry, $data = null, $updatetime = true, $allowfinalfieldedit = false) {
        ...
        if ($entry->id > 0) {
            if ($dlx->user_can_manage_entry($entry, $allowfinalfieldedit)) { // Just in case two forms are open.
                ...
                // Stamp the final-submission date exactly once and never overwrite it afterwards.
                $oldsubmitted = $DB->get_field('datalynx_entries', 'timesubmitted', ['id' => $entry->id]);
                if (
                    empty($oldsubmitted)
                        && (int) $entry->status === datalynxfield_status::STATUS_FINAL_SUBMISSION
                ) {
