...
$df->set_page('customfilter/index', array('modjs' => true, 'urlparams' => $urlparams));

navigation_node::override_active_url(
        new moodle_url('/mod/datalynx/customfilter/index.php', array('id' => $df->cm->id)));

$fm = $df->get_customfilter_manager();
...
if ($urlparams->new and confirm_sesskey()) {
    ...
    if (!$filters = $fm->get_filters(null, false, true)) { // Any notifications?
        $df->notifications['bad'][] = get_string('customfiltersnoneindatalynx', 'datalynx');
    }
    // Print header.
    $df->print_header(array('tab' => 'customfilters', 'urlparams' => $urlparams));
